...
def _apply_overlays_and_audio(concat_video, raw_video, bg_music, duration, output_path):
    ...
    Graphics:
      ...
      • Like prompt    (~22s–28s): Side box + "👍 Like this video!"
      • Outro          (last 8s): Bottom banner + 3 lines of text
    """
    d = duration
    ...
    fc = (
        ...
        f"[v10]drawbox="
            ...
            f"enable='between(t,{outro_start-0.5:.2f},{d:.2f})'[v11];"
...
def handler(event):
    ...
    with tempfile.TemporaryDirectory(prefix="bmm_") as work_dir:
        ...
        download_file(BG_MUSIC_URL,     music_path)
